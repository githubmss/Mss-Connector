...
class SetUserinfo extends \Magento\Framework\App\Action\Action
{
        ...
        if ($this->customerSession->isLoggedIn()) {
            ...
            if (isset($data)) {
                ...
                if (isset($data['profile_image'])) {
                    $imageResult = $this->base64Convert($data['profile_image'], $data['type']);
                    $this->checkData($imageResult);
                    $customerData = $customer->getDataModel();
                    $customerData->setCustomAttribute('mss_profile_image_customer', $this->storeManager
                            ->getStore()->getBaseUrl() .
                        $imageResult['file_url']);
                    $customer->updateData($customerData);
                    try {
                        $objectData = \Magento\Framework\App\ObjectManager::getInstance();
                        $customRepo = $objectData
                            ->get('\Magento\Customer\Model\ResourceModel\CustomerFactory')->create();
                        $customRepo->save($customer, 'mss_profile_image_customer');
                    } catch (\Exception $e) {
     ...
     * @return Array
     */
    private function base64Convert($base64_string, $type)
    {
        define('UPLOAD_DIR', 'pub/profile_image/');
        if (!$this->isExists('pub/profile_image')) {
            $io->mkdir($this->directoryList->getPath('pub') . '/profile_image', 0775);
        }
        try {
